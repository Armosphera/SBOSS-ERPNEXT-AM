"""AE VAT 201 — FTA-mandated UAE VAT return form (W2-T13).
...
Mirrors ``frappe_armenia.armenia.report.am_vat_return`` (W1-T15) so the
two apps look symmetric and easy to extend. UAE specifics:
  - Currency: AED (vs AMD for Armenia)
  - Standard rate 5% (Federal Decree-Law No. 8 of 2017)
  - Filing period: Quarterly (per Cabinet Decision 52/2017)
  - This report aggregates the totals; the per-FTA-box breakdown
    (Box 1 standard-rated, Box 3 zero-rated exports, Box 4 exempt,
    Box 5 reverse-charge imports) lives in W2-T14/W2-T15 register
    entries.

The pure helper :func:`compute_ae_vat_return` is exposed for unit
testing. The Frappe-facing :func:`execute` wraps it as a Script
Report.
"""
...
def compute_ae_vat_return(
    ...
    -------
    dict
        ``output_vat``, ``input_vat``, ``net_vat`` (all floats in AED),
        plus ``output_invoice_count`` and ``input_invoice_count`` (ints).

    Notes
    -----
    - Only submitted invoices (``docstatus = 1``) are included.
    - Tax amounts come from each invoice's ``total_taxes_and_charges``
      column — same source the ``validate_sales_invoice_vat_uae`` hook
      checks against.
...
def _aggregate_vat(doctype: str, company: str, from_date: str, to_date: str) -> tuple[float, int]:
    ...
    Uses raw SQL with ``COALESCE(SUM(...), 0)`` so the result is always
    a row even when no invoices match. Returns ``(vat_sum, count)``.
    """
...
def execute(filters: dict | None = None):
    ...
    summary = compute_ae_vat_return(company, from_date, to_date)

    columns = [
        {"label": "Section", "fieldname": "section", "fieldtype": "Data", "width": 280},
        {"label": "Amount (AED)", "fieldname": "amount", "fieldtype": "Currency", "width": 180},
    ]
